...
| **4** ✅ | `@4dl/commerce` — budget math, the customer-lapse ladder, discount codes, 6 tables. The scope list and the lapse COPY are the app's; the `wrong_client` wire code became `wrong_subject` and the rename landed in two screens with it. | Sits beside billing, not on it: an app can take billing without commerce, and vice-versa. |
...
| **6** | `@4dl/email`, `@4dl/notify`, `@4dl/storage` | Independent of each other; can run in parallel. |
