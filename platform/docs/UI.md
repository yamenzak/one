...
| `pane-container-queries` | a viewport query anywhere in the structure. It used to allow ONE — the shell's own columns — and the exemption was the bug: a shell knows its width, so asking the monitor as well let the two disagree, and a phone-width shell in a narrow container took a rail column and put its bottom island in the top row | **live** |
...
| `every-component-has-been-drawn` | a registered component no screen in the sample app uses. Read back out of the RENDERED markup rather than from a list each specimen declared itself into, so it proves a component was drawn rather than that somebody wrote its name | **live** |
| `nothing-is-drawn-that-is-not-declared` | a family of parts with no component at its root — an unregistered component wearing an established one's clothes, so it has no state matrix and the boundary does not know it exists | **live** |
| `the-refusals-are-drawn-too` | a state the language declares that the sample app never reaches — locked, busy, invalid, or a container that failed. These are what a product ships most and reviews least, and their COPY can only be judged where the state occurs | **live** |
| `every-group-states-its-rule` | a screen with no stated reason for its archetype, or an archetype no screen demonstrates. A sample app whose screens have no reasons is a picture book, and the next person copies by appearance | **live** |
...
| `every-registered-component-is-drawn` | a component on the registry that no renderer emits, or that the sheet never addresses — the defect every other test in the package structurally cannot see, because they ask what the markup MEANS and an unstyled element means the right thing | **live** |
| `every-emitted-part-is-drawn` | a data-one attribute the stylesheet never mentions — it renders as a bare box in normal flow, which is how the overlay, the navigation and the skeleton each shipped unstyled with a green suite | **live** |
| `no-rule-for-a-part-nothing-emits` | an affordance that was designed, written and never rendered — the row's disclosure chevron had a rule for four stages and no element, so every tappable row looked identical to every inert one | **live** |
| `a-drawing-exemption-is-checked` | a part excused because a borrowed class dresses it, where the borrow is gone or the sheet draws it after all — an exemption that outlives its reason is worse than no exemption, because it reads as a decision | **live** |
| `every-declared-state-is-drawn` | a state the registry declares and no rule renders — busy set the native disabled flag and drew nothing, so a control mid-request was pixel-identical to one that is simply unavailable | **live** |
| `every-masked-sky-breathes-on-its-own-period` | a mask with no weather, which is the one visibly still thing on a screen, or two skies on one period, which is visibly one animation | **live** |
| `every-sky-offered-is-a-sky-drawn` | a word in the closed set with no rule — the attribute lands, nothing matches it, and the page renders no light at all | **live** |
| `a-part-rule-never-outruns-its-icon` | a positional rule counting past the parts a generated glyph has, which animates the wrong stroke or nothing at all | **live** |
| `no-backtick-inside-a-sheet-named-where-it-is` | a backtick written in a comment inside a CSS template literal — it closes the sheet, and the compiler then reports a missing , or ; hundreds of lines away in a line that has nothing wrong with it. Scans raw text, so it reports the cause on a file that no longer parses | **live** |
| `the-top-of-the-ladder-casts-a-shadow` | a rule reaching the deepest surface with no elevation beside it. A light ground starts at l 0.93 and the ladder saturates, so surface-3 and surface-2 come out one unit apart on every pale tenant — this one check found six components that were invisible in light and correct in dark, including the segmented indicator and the current destination on a nav island | **live** |
| `the-backdrop-is-a-length-not-a-proportion` | a solid/reach emitted as a percentage. As a proportion of the page these measured how much CONTENT a screen happened to have — the same archetype washed a long scroll end to end and barely tinted a short one, and at a desktop width every screen came out tinted throughout | **live** |
