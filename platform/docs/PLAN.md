...
| `the-account-has-its-own-terms` | an account holder who belongs to no product being asked to agree to nothing at all. Every other document here is owed for being an owner or a client of something; an address, a passkey and a vault are held for having an account, and that is the obligation nobody could express until there was a role for it | **live** |
| `the-deployment-declaration-survives-composition` | the one set of documents everybody is asked for being the one set nothing checks. `defineApp` runs `legalProblems` over every app's; the deployment's is imported rather than composed, so a document with no version or no text would ship past the check that exists to refuse it | **live** |
| `the-account-privacy-notice-covers-the-vault` | a vault nobody's privacy notice speaks for. A vault fact outlives every workspace somebody leaves — that is what the vault IS — so an app's notice structurally cannot cover it, and a separate vault document would imply you could decline it and keep the account | **live** |
| `every-worker-passes-the-declaration` | an app built from the template that never passes `deployment`, whose account holders are therefore asked for no terms at all. Unpassed it is `null` and `legal.list` merges nothing — every suite green, the screen showing each product's documents, and the account's own missing with nothing saying so | **live** |
...
| `every-write-is-recorded-by-default` | an audit trail that is a thing people remember rather than a thing the platform does. `auditFor` returned null for any operation declaring nothing, so twenty of the platform's own writes were recorded NOWHERE while every suite stayed green — because a missing entry is indistinguishable from an action nobody took. Absent now derives one: the verb from the operation's own id, the subject from its row scope or its input's id | **live** |
| `an-audit-exemption-states-why` | silence passing for a decision. A handful of writes genuinely keep their own record — the vault has a grant log, a sign-in has a session — and duplicating those into the audit is noise that makes the real entries harder to find. `{ why }` is the same shape `tool` uses, for the same reason: saying so out loud is cheap, being silently absent is what it replaced | **live** |
| `reads-are-not-recorded-unless-they-ask` | every list, every poll and every screen load becoming an entry — an audit nobody can read, which is the same silence as no audit with a much larger bill. A read that genuinely matters declares its own | **live** |
| `a-write-says-what-to-record-or-why-not` | a derived entry taken as a licence to stop thinking. Coverage is automatic now, so this is not about coverage — it is about the two ways a derived entry is WORSE: `commerce.days` derives `verb: "days"`, which says nothing about access being extended, and an operation that keeps its own record elsewhere gets a duplicate nobody wants | **live** |
| `an-action-taken-by-a-model-is-told-apart` | an audit that cannot answer the first question anybody asks about an agent. The tool path is the SAME runner — `dispatch` calls `run(target, payload, "tool")` — so the gates, the row scope, the quota and the audit are identical, and the only difference is the one worth recording | **live** |
| `the-api-describes-itself` | `openApiFor` written, unit-tested and served by NOTHING — every route, its method, its input and its declared failures derivable from the one registry, and no caller able to read any of it. A document that exists beside an API that is undocumented | **live** |
| `the-api-document-is-filtered-by-the-caller` | a document listing operations the reader's first call refuses, which is a support ticket rather than a reference. It is the same `check` over the same registry the router and the tool catalogue use, so there is no second list to keep in step | **live** |
| `a-token-is-the-persons-reach-not-a-copy-of-it` | a scope list frozen at issue — the shape where somebody loses a role and their token keeps it, silently, until an audit finds a machine doing what its owner no longer may. A token stores no permissions at all: it names an account and a workspace, and every request resolves that member's reach live, exactly as a browser session does | **live** |
| `a-token-proves-nothing-recent` | the one way past the proof gate. `proof: "recent"` exists against a borrowed laptop with an open tab and the answer is a code to an inbox, which a machine cannot give — so closing an account, moving money and minting another credential stay things a person does in front of a screen. `provenAt` is null on a token session and the gate reads that as not proven | **live** |
| `a-token-names-a-workspace-its-owner-is-in` | a credential naming a workspace somebody left, or never joined, which the resolver would happily honour. The tenant id is something a caller types | **live** |
...
they are about one workspace's own records and live where those records live —
so the worker serving the hub answers for its own product and no other. The
membership index crosses that boundary because a membership is routing; settings
are DATA, and publishing them into the shared store would put one business's
configuration where another product's worker can read it. The hub says where a
workspace is managed instead, and `a-hub-says-what-it-cannot-answer` keeps it
from quietly rendering an empty card in place of an answer.
