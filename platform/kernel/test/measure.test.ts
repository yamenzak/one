/**
 * THE ARITHMETIC THAT IS WRONG IN HALF THE CASES.
 *
 * Half of all targets count DOWNWARDS, and the obvious formula is silently
 * backwards for every one of them — it reports somebody who has not moved as
 * past the finish, and somebody going the wrong way as doing better.
 */

import { describe, expect, it } from "vitest";
import { consistency, daysLeft, expectedOver, meanOverRecorded, progressOf, scoreOf } from "../src/measure.js";

describe("a goal that counts upwards", () => {
  it("is measured from the baseline towards the target", () => {
    expect(progressOf({ start: 60, current: 70, target: 80 })).toMatchObject({ direction: "up", done: 0.5, moved: 10, reached: false });
  });

  it("has covered nothing before anybody moves", () => {
    expect(progressOf({ start: 60, current: 60, target: 80 }).done).toBe(0);
  });

  it("reports what is left in the same units", () => {
    expect(progressOf({ start: 60, current: 70, target: 80 }).toGo).toBe(10);
  });
});

describe("a goal that counts downwards", () => {
  /*
    ⚠️ THE NAIVE `current / target` SAYS SOMEBODY AT 90 KG AIMING FOR 75 IS 120%
    DONE BEFORE THEY HAVE LOST A GRAM — and that somebody who GAINED is doing
    better. Measuring from the baseline is the only formula that reads correctly
    in both directions, and it is why a goal records where somebody started.
  */
  it("is measured the same way, and is not 120% done on day one", () => {
    const dayOne = progressOf({ start: 90, current: 90, target: 75 });
    expect(dayOne).toMatchObject({ direction: "down", done: 0, reached: false });
  });

  it("counts losing as progress", () => {
    expect(progressOf({ start: 90, current: 82.5, target: 75 }).done).toBe(0.5);
  });

  /* ⚠️ And moving the wrong way is not progress, however the sign works out. */
  it("counts gaining as none, rather than as negative progress on a bar", () => {
    const wrongWay = progressOf({ start: 90, current: 94, target: 75 });
    expect(wrongWay.done).toBe(0);
    expect(wrongWay.moved).toBe(4);
  });
});

describe("reaching it", () => {
  /*
    ⚠️ REACHED MEANS PAST IT, NOT EQUAL TO IT. Somebody aiming for 75 who gets to
    74 has reached it; an equality check leaves them at 99% for overshooting.
  */
  it("is reached on arrival and stays reached past it", () => {
    expect(progressOf({ start: 90, current: 75, target: 75 }).reached).toBe(true);
    expect(progressOf({ start: 90, current: 74, target: 75 })).toMatchObject({ reached: true, done: 1, toGo: 0 });
  });

  /*
    ⚠️ A TARGET EQUAL TO THE BASELINE IS "HOLD", not a division by zero. Keeping
    a weight is a real goal, and it is reached while somebody is still there.
  */
  it("holds a weight rather than dividing by nothing", () => {
    expect(progressOf({ start: 80, current: 80, target: 80 })).toMatchObject({ direction: "hold", reached: true, done: 1 });
    expect(progressOf({ start: 80, current: 83, target: 80 })).toMatchObject({ direction: "hold", reached: false, done: 0, toGo: 3 });
  });
});

describe("how long is left", () => {
  it("counts whole days to the date", () => {
    expect(daysLeft("2026-08-11", "2026-08-18")).toBe(7);
    expect(daysLeft("2026-08-11", "2026-08-11")).toBe(0);
  });

  /*
    ⚠️ NEGATIVE IS OVERDUE AND IS NOT CLAMPED. "0 days left" and "11 days late"
    are different things to say to somebody, and a product that clamps says the
    gentler one forever.
  */
  it("goes negative rather than resting at zero", () => {
    expect(daysLeft("2026-08-22", "2026-08-11")).toBe(-11);
  });

  it("answers nothing rather than NaN for a date it cannot read", () => {
    expect(daysLeft("2026-08-11", "soon")).toBe(0);
  });
});

/* --------------------------------------------------------------- a score --- */

/*
  ⚠️ ONE NUMBER FROM SEVERAL IS WHERE PRODUCTS LIE BY ACCIDENT. Treating an
  absent input as zero punishes somebody for not recording, which is precisely
  backwards: the periods people record least are the hard ones, so the score
  falls hardest exactly when it is read most.
*/
describe("a score from several inputs", () => {
  const part = (name: string, value: number) => ({ name, value });

  it("averages what is present rather than dividing by what could have been", () => {
    /* Two perfect inputs out of five is 100, not 40. */
    expect(scoreOf([part("a", 1), part("b", 1)], 5).score).toBe(100);
  });

  /* ⚠️ Below the minimum there is no score at all: one input is not a picture,
     and a number computed from one is one somebody will act on. */
  it("declines to give one below the minimum", () => {
    expect(scoreOf([part("a", 1)], 5).score).toBeNull();
    expect(scoreOf([], 5).score).toBeNull();
  });

  /* ⚠️ And the coverage travels with the number, or 82 from one input looks
     exactly like 82 from five. */
  it("says what it was computed from either way", () => {
    expect(scoreOf([part("a", 1)], 5)).toMatchObject({ from: ["a"], coverage: 0.2 });
    expect(scoreOf([part("a", 1), part("b", 0)], 4)).toMatchObject({ from: ["a", "b"], coverage: 0.5 });
  });

  it("clamps a part somebody computed past the ends", () => {
    expect(scoreOf([part("a", 3), part("b", -1)], 2).score).toBe(50);
  });
});

/* ---------------------------------------------------------- how much of --- */

describe("how much of what was asked for happened", () => {
  it("is a ratio when something was asked for", () => {
    expect(consistency(3, 4).ratio).toBe(0.75);
  });

  /*
    ⚠️ NOTHING EXPECTED IS NOT ZERO PER CENT. Somebody who was asked for nothing
    has not failed to do it, and a screen showing them at 0% is an accusation
    about work nobody ever set.
  */
  it("is null when nothing was", () => {
    expect(consistency(0, 0).ratio).toBeNull();
    expect(consistency(5, 0).ratio).toBeNull();
  });

  it("does not go past one for somebody who did more", () => {
    expect(consistency(9, 4).ratio).toBe(1);
  });

  it("turns a weekly rate into a window, and a null rate into nothing", () => {
    expect(expectedOver(3, 28)).toBe(12);
    expect(expectedOver(null, 28)).toBe(0);
    expect(expectedOver(3, 0)).toBe(0);
  });
});

/* -------------------------------------------------- over what was there --- */

describe("an average over what was recorded", () => {
  const day = (a: number, b: number) => ({ a, b });
  const recorded = (d: { a: number; b: number }) => d.a > 0 || d.b > 0;

  /*
    ⚠️ DIVIDED BY THE DAYS WITH ENTRIES, NOT BY THE WINDOW. Dividing by the
    window turns three carefully recorded days into a number that is wrong,
    alarming, and produced by mistaking silence for absence.
  */
  it("divides by the days with entries", () => {
    const out = meanOverRecorded([day(10, 4), day(0, 0), day(20, 6), day(0, 0)], recorded);
    expect(out.days).toBe(2);
    expect(out.mean).toEqual({ a: 15, b: 5 });
  });

  /* ⚠️ Null rather than a confident row of zeros — a mean of nothing is
     unanswerable, and every caller has to decide what to say about that. */
  it("answers null where nothing was recorded", () => {
    expect(meanOverRecorded([day(0, 0)], recorded)).toEqual({ mean: null, days: 0 });
  });
});
