...
/** What the account centre shows about one fact. Never a value it may not see. */
export interface FactState {
  readonly fact: string;
  readonly label: string;
  readonly held: boolean;
  readonly grants: readonly {
    readonly appId: string; readonly tenantId: string | null;
    readonly reach: Reach; readonly expiresAt: Instant | null; readonly live: boolean;
  }[];
}

export async function stateOf(store: VaultStore, accountId: UserId, now: Instant): Promise<readonly FactState[]> {
  const [grants, held] = await Promise.all([store.grants(accountId), store.holds(accountId)]);
  const byFact = new Map<string, Grant[]>();
  for (const g of grants) byFact.set(g.fact, [...(byFact.get(g.fact) ?? []), g]);

  const seen = new Set([...held, ...byFact.keys()]);
  return [...seen].flatMap((id) => {
    ...
    return [{
      ...
      label: fact.label,
      held: held.has(id),
      grants: (byFact.get(id) ?? []).map((g) => ({
        appId: g.appId,
        tenantId: g.tenantId,
        reach: g.reach,
        expiresAt: g.expiresAt,
        /* ⚠️ Computed here rather than filtered out. A lapsed grant is something
           a person should SEE, with its date — hiding it makes "it ran out" and
           "I never gave it" the same screen. */
        live: g.expiresAt === null || g.expiresAt > now,
      })),
    }];
  });
}
