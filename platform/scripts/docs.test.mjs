...
if (current !== inventory) {
  ...
  else fail(`docs/DEFERRED.md is out of date. Run \`node platform/scripts/docs.test.mjs --write\`.`);
}
...
/*
  ⚠️ THE STATE BLOCK IS WHAT MAKES RESUMING MECHANICAL.

  Whoever picks this up next — after a fortnight, or after a conversation's
  context was compressed — reads this file first, and it cannot be stale because
...
const stateBlock =
  `## Where the build is\n\n` +
  `| | |\n|---|---|\n` +
  `| active | ${activeStages.length ? activeStages.map((x) => `**${x.id}** — ${x.title}`).join("<br>") : "none"} |\n` +
  `| shipped | ${shippedStages.length ? shippedStages.map((x) => x.id).join(", ") : "none yet"} |\n` +
...
if (idxCurrent !== indexDoc) {
  ...
  else fail(`docs/README.md is out of date. Run \`node platform/scripts/docs.test.mjs --write\`.`);
