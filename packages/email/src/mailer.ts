...
/**
 * The PLATFORM sender — the address the platform's own mail (and any tenant on the
 ...
 */
/**
 * Where platform mail comes from when nothing is configured.
 *
 * ⚠️ Deliberately an ADDRESS THAT CANNOT SEND. A plausible-looking default is
 * worse than an obviously broken one: it would let a fresh deploy appear to work
 * while every message silently bounces at the provider. The app sets its own via
 * `configureEmailBrand`, and DEPLOY documents `email.platform_from`.
 */
...
function buildMime(m: { from: string; to: string; subject: string; html?: string; text?: string }): string {
  ...
  const domain = bareAddress(m.from).split("@")[1] ?? "localhost";
...
/**
 * The PLATFORM's own identity — sign-in codes, receipts, dunning.
 *
 * Distinct from a TENANT's brand: those emails come from us, not from them, and
 * wearing a tenant's colours on a "your subscription lapsed" message would be a
 * small lie. Set once with `configureEmailBrand`; the fallback below is
 * deliberately anonymous rather than pretending to be a product.
 */
export const DEFAULT_PLATFORM_BRAND: BrandKit = { name: "", accent: "#a8c7fa", accentFg: "#0b1220", logoUrl: null };

let platformBrand: BrandKit = DEFAULT_PLATFORM_BRAND;
let platformFrom: string = PLATFORM_FROM_DEFAULT;

/** Bind the platform's identity once, at worker start. */
export function configureEmailBrand(brand: BrandKit, from: string): void {
  platformBrand = brand;
  platformFrom = from;
}

/** The platform's brand, as configured. */
export const platformBrandKit = (): BrandKit => platformBrand;
export const platformFromAddress = (): string => platformFrom;
...
export function emailButton(label: string, href: string, brand: BrandKit = platformBrandKit()): string {
...
export function emailShell(
  ...
  const footnote = opts.footnote ?? `Sent by ${escapeHtml(isPlatform ? platformBrandKit().name : brand.name)}.`;
...
const brandAccent = (brand?: BrandKit) => brand?.accent ?? platformBrandKit().accent;
