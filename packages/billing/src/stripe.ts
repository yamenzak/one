...
/**
 * How this app labels the objects it creates in Stripe.
 *
 * ⚠️ **`metadataPrefix` is LIVE DATA, not a display string.** It becomes
 * `metadata[<prefix>_tenant]` on every customer, checkout session and
 * subscription, and the webhook handlers read those keys back to decide which
 * tenant a payment belongs to. Changing it on a deployment that already has
 * customers orphans every one of them: the events still arrive, the lookup finds
 * nothing, and payments succeed while nothing is activated. Pick it once.
 *
...
/** Metadata keys for a tenant/plan/pack, given the app's prefix. */
export const stripeMetaKeys = (b: Pick<StripeBranding, "metadataPrefix">) => ({
  tenant: `metadata[${b.metadataPrefix}_tenant]`,
  plan: `metadata[${b.metadataPrefix}_plan]`,
  pack: `metadata[${b.metadataPrefix}_pack]`,
  /** The same three, nested under a checkout session's `subscription_data`. */
  subTenant: `subscription_data[metadata][${b.metadataPrefix}_tenant]`,
  subPlan: `subscription_data[metadata][${b.metadataPrefix}_plan]`,
});
...
export async function ensureCustomer(
  ...
  secretKey: string,
  tenantId: string,
  branding: Pick<StripeBranding, "metadataPrefix">,
  email?: string,
): Promise<string> {
  ...
  const customer = await stripeCall<{ id: string }>(secretKey, "customers", {
    email,
    [stripeMetaKeys(branding).tenant]: tenantId,
  });
