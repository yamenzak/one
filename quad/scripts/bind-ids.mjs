#!/usr/bin/env node
/**
 * WRITE REAL D1 IDS INTO `quad/one/wrangler.jsonc`, AND READ BACK WHETHER IT IS
 * PROVISIONED.
 *
 * ⚠️ QUAD DOES NOT USE `apps.json`, AND MUST NOT START. The legacy binder
 ...
 *
 * Usage:
 *   node quad/scripts/bind-ids.mjs <database_name> <uuid>   bind one id
 *   node quad/scripts/bind-ids.mjs --ready                  exit 0 if every id is real
 *   node quad/scripts/bind-ids.mjs --names                  print every database_name
 */
...
if (first === "--ready") {
  ...
  if (!rows.length) {
    console.error("BAD  quad/one/wrangler.jsonc binds no D1 database at all.");
    process.exit(1);
  }
...
if (!first || !second) {
  console.error("Usage: node quad/scripts/bind-ids.mjs <database_name> <uuid> | --ready | --names");
  process.exit(2);
}
