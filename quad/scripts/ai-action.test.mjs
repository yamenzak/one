 ...
 * them belongs.
 */
const apps = readdirSync(join(QUAD, "apps"), { withFileTypes: true })
  .filter((d) => d.isDirectory())
  .map((d) => [d.name, join(QUAD, "apps", d.name, "src", "index.ts")]);
...
const operator = strip(readFileSync(join(QUAD, "runtime/src/operator.ts"), "utf8"));
const settings = strip(readFileSync(join(QUAD, "runtime/src/settings.ts"), "utf8"));
const actions = strip(readFileSync(join(QUAD, "runtime/src/ai-actions.ts"), "utf8"));
...
if (unchecked.length) {
  ...
  strip(readFileSync(join(QUAD, "kernel/src/ai.ts"), "utf8")))) {
 ...
 * anybody was shown.
 */
const services = strip(readFileSync(join(QUAD, "runtime/src/services.ts"), "utf8"));
