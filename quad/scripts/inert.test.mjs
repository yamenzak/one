/**
 * QUAD CANNOT REACH THE PRODUCTION DEPLOYMENT, AND NOTHING IT BINDS IS
...
const registered = apps.filter((a) => (a.dir ?? "").startsWith("quad/"));
if (registered.length) {
  fail(`apps.json registers ${registered.map((a) => a.id).join(", ")} under quad/.\n` +
  ...
  ok(`registry: apps.json names nothing under quad/, so deploy.yml cannot select it`);
}
...
if (!worker) {
  fail(`quad/one/wrangler.jsonc declares no worker name.`);
...
if (!root) {
  fail(`quad/one/wrangler.jsonc sets no ROOT — every hostname would classify as no door.`);
...
console.log(bad
  ? `\ninert: ${bad} finding(s) — quad can reach the live deployment.`
  : `\ninert: quad is off the production deploy path, and shares no name with it.`);
