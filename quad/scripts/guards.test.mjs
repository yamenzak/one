...
const ROOT = join(QUAD, "..");

let bad = 0;
const fail = (m) => { console.error(`BAD  ${m}`); bad++; };
const ok = (m) => console.log(`ok   ${m}`);

const registry = JSON.parse(readFileSync(join(QUAD, "docs/guards.json"), "utf8"));
const guards = registry.guards;
const decisions = readFileSync(join(QUAD, "docs/DECISIONS.md"), "utf8");
...
const commands = [scripts["quad:gate"] ?? "", scripts["quad:test"] ?? "", scripts["quad:typecheck"] ?? ""].join(" ");
for (const g of live) {
  /* A vitest file is reached by `quad:test`; an .mjs guard has to be named. */
  if (g.impl.endsWith(".mjs") && !commands.includes(g.impl.replace(/^/, "quad/"))) {
    fail(`${g.id}: ${g.impl} is not named by quad:gate, so it never runs`);
  }
}
