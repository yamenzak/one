...
for (const file of SOURCES) {
  ...
  for (const [, what] of src.matchAll(/(cubic-bezier\s*\([^)]*\))/g)) {
    ...
    fail(`${name}: writes its own curve — ${what}.\n` +
         ...
         `       them. Use \`EASE\` / \`MOTION\` from @quad/design.`);
  }
...
const typeSrc = readFileSync(join(QUAD, "design/src/tokens/type.ts"), "utf8");
 ...
 */
const TYPE_SRC = readFileSync(join(QUAD, "design/src/tokens/type.ts"), "utf8");
 ...
 */
const AMBIENCE = readFileSync(join(QUAD, "design/src/tokens/ambience.ts"), "utf8");
