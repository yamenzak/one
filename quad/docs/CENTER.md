...
**The platform UI, rebuilt on Quad: one tenant centre for every app a workspace
...
AFTER (quad):     one operation declares   permission + entitlement
...
One shell behind a workspace's own address, and the first Quad surface that
