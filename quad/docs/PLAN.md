...
3. **What is not built yet says so, in a marker a script can find** —
   `DEFER(quad-N)` — never in prose somebody has to remember. See
...
⚠️ **AND THE PREDECESSOR IS `platform/`, WHICH STILL WORKS.** Quad is not a
...
architecture Quad deliberately departs from in the ways §2 sets out.

---

## 1. What changed, and why there is a second attempt

`platform/` (the `@one/*` tree) got the mechanisms right and the **topology**
wrong. Three consequences, and each is the reason for one of Quad's decisions:
...
Quad inverts all three: **the tenant is primary, apps are enabled on it, and
...
```
<slug>.quad.4dl.app        a tenant. Every app it has, from one address.
...
```
@quad/kernel    Pure. Types, rules, decisions-as-functions. No I/O, no bindings,
...
| # | Stage | Ends when |
|---|---|---|
| 0 | **Ground** — workspace, docs, guard registry, the standards that bind them | `pnpm quad:gate` runs and passes with the first guards live |
...
| 9 | **Kova** — the first real product on Quad | Kova runs with no app-specific infrastructure |
