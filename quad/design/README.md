...
⚠️ **The name is `OneDesign`; the package is `@quad/design`, and the two differ on
purpose.** *One* is the DEPLOYMENT — what a customer types. *Quad* is the
FRAMEWORK — what a contributor imports. Every app imports this, including Quad's
own reference apps, so it is scoped to the framework; the name is the family it
was designed for. Naming the package `@one/*` would bind the design system to one
deployment, which is the exact split `quad/docs/DECISIONS.md` exists to keep.

**One public surface: `import { … } from "@quad/design"`.** There is no deep
...
- **No deep import.** `@quad/design` has one entry; reaching into `src/` is how a
