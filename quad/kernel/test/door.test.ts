...
const ROOTS = { root: "quad.4dl.app" };

describe("the doors under our own root", () => {
  it("knows each one by its label", () => {
    expect(doorFor("quad.4dl.app", ROOTS).kind).toBe("signpost");
    expect(doorFor("id.quad.4dl.app", ROOTS).kind).toBe("account");
    expect(doorFor("admin.quad.4dl.app", ROOTS).kind).toBe("operator");
    expect(doorFor("setup.quad.4dl.app", ROOTS).kind).toBe("setup");
  });

  it("reads a workspace's slug off its address", () => {
    const door = doorFor("Northwind.quad.4dl.app", ROOTS);
    expect(door).toEqual({ kind: "tenant", slug: "northwind", host: "northwind.quad.4dl.app" });
  });
  ...
  */
  it("refuses the labels that are infrastructure", () => {
    for (const label of ["acme", "acme-challenge", "autodiscover", "_domainkey", "mail", "www"]) {
      expect(doorFor(`${label}.quad.4dl.app`, ROOTS).kind).toBe("none");
    }
  });
  ...
  /*
    ⚠️ ONE LABEL ONLY. Treating `a.b.quad.4dl.app` as a workspace called "a.b"
    makes a wildcard certificate a way to invent addresses under our own name.
  */
  it("does not serve a second level", () => {
    expect(doorFor("a.b.quad.4dl.app", ROOTS).kind).toBe("none");
  });

  it("refuses a label that could not be a DNS name", () => {
    expect(doorFor("Not_A_Label.quad.4dl.app", ROOTS).kind).toBe("none");
    expect(doorFor("-leading.quad.4dl.app", ROOTS).kind).toBe("none");
  });
});
...
describe("everything else", () => {
  ...
  */
  ...
  it("only opens the device door where a deployment asked for one", () => {
    expect(doorFor("play.quad.4dl.app", ROOTS).kind).toBe("none");
    expect(doorFor("play.quad.4dl.app", { ...ROOTS, device: true }).kind).toBe("device");
  });

  it("ignores a port", () => {
    expect(doorFor("northwind.quad.4dl.app:8787", ROOTS).kind).toBe("tenant");
  });

  it("says which doors resolve a workspace at all", () => {
    expect(needsTenant(doorFor("northwind.quad.4dl.app", ROOTS))).toBe(true);
    expect(needsTenant(doorFor("admin.quad.4dl.app", ROOTS))).toBe(false);
  });
});
