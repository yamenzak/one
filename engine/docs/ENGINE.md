...
| `surface` | a screen declared — its shape, its blocks, and the closed vocabulary they bind through | 29 | — |
| `blocks` | which blocks a screen may name at all | 3 | — |
| `manifest` | the whole app, and the composition that refuses a broken one | 19 | — |
...
| `a-slot-is-a-prop-the-component-actually-takes` | D92 | a registry entry naming a slot its component does not accept - React drops an unknown prop with no error, no warning and no type complaint, so the renderer fills `rows` on a component that reads `of`, the manifest composes, the screen mounts, and the region is blank; twenty-three of the forty entries were wrong that way when the renderer first needed the join |
| `a-block-in-the-vocabulary-is-placed-by-a-declaration` | D92 | a registry that grows entries nothing composes - the slot-to-prop join is asserted statically and no render ever makes it good, so the entry is correct on paper and draws an empty region the first time an app names it; thirteen charts and six list shapes were registered that way, chosen by listing what the design package exports rather than by counting what a product draws |
| `a-file-a-product-draws-with-is-rendered-by-something` | D92 | a hand-written screen left inside a product with its wiring already done - no route names it, no session map holds it and no harness photographs it, so every check here reports ok while it sits one import away from being live again; that is the shape somebody reaches for when a declaration will not say what they want, and 210 lines of it survived the surface being emptied |
| `a-declaration-in-a-screen-out` | D92 | a binding that resolves and never arrives - a value read correctly out of the record and handed to a prop nothing reads, a format that never reaches a formatter so every price ships as a bare integer of minor units, a condition that draws a block it should have left out, and a split whose aside was left in the main column |
...
| `every-product-screen-is-declared-or-says-what-it-waits-for` | D92 | a product growing a hand-written screen on an afternoon nobody reviewed - every other check in this directory reads a .tsx and reports ok over it, so the arc loses ground behind a green run; plus an exemption that outlives its reason, either because the screen it names became a declaration and the entry stayed, or because it names a screen no product declares and covers nothing at all |
...
| D92 | A screen is a body or a story, and what a body binds through is closed | 19 |
