...
1. **Nothing is deployed to Cloudflare — but the workflow that would is built.**
   `.github/workflows/engine.yml` gates, provisions and deploys One; the only thing
   left is a run. The D1 ids in `engine/one/wrangler.jsonc` are placeholders, and
   the deploy SKIPS on them rather than shipping a worker bound to nothing —
   Actions → **OneEngine** → Run workflow with `provision` ticked is what creates the
   ...
   - ⚠️ **The isolation from the live product is a guard, not a habit.**
     `engine/scripts/inert.test.mjs` fails if One is registered in `apps.json` (the
...
**`@engine/runtime` — the directory, the shards, and a schema nobody versions.**
...
**`@engine/hello` — the smallest complete app, and the reference.** A manifest and
...
**`@engine/design` — the screens nobody writes.** HeroUI v3, router-free.
...
- **Every ground comes from one engine, governed by
  [AMBIENCE.md](../design/AMBIENCE.md)** — seven FAMILIES, each seeded, replacing the
  twenty-four hand-written ambiences entirely. Four belong to a SUBJECT (a
  workspace, a person, a product, the deployment) and three a screen may name;
  why `plain` is still the default rather than a failure to choose is there too.
...
OneEngine surface with a real router, and it is still a pure parse: five areas —
...
| D7 | HeroUI v3 is the component layer, and its components are not restyled | 53 |
...
pnpm engine:test         every suite
pnpm engine:gate         the guards: docs, registry, layers
node engine/scripts/docs.test.mjs --write    refresh the generated blocks above
```
