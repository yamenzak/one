...
| **The Announced Heading** | A name that exists, is filled in correctly, and is drawn nowhere. A `Listing`'s `label` is the table's ACCESSIBLE name and a chart's `describes` was only ever a `<title>` — so a page carrying two lists showed two sets of rows in the same ink with nothing saying which was which, and every chart in the vocabulary was an unlabelled plot. On a kit page it put "Towel clamp · 2" — what is MISSING — directly under two items that are in the tray. Invisible to every check that reads the declaration, because the declaration is right. Five screens, found in one photograph. | `surface.ts` — `lists_unheaded`; `charted` — every chart form draws its name and still announces it |
