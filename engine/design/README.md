...
**394 exports.** Every one is reachable as `import { … } from "@engine/design"`;
there is no deep import, and a guard says so.

### `tokens/`

- `tokens/ambience.ts` — `ambienceStylesheet`, `FADE`, `GRAIN`, `GRAIN_OPACITY`, `LIGHT`, `MATTE`, `ON_SCENE`, `REACH`, `skyWorld`, `World`, `worldCss`
...
- `tokens/motion.ts` — `applyLiveliness`, `ARRIVE`, `ARRIVE_MARK`, `ARRIVE_MOTION`, `ARRIVE_RISE`, `arriveAt`, `BEAT`, `BLOCK_MOTION`, `Device`, `DOOR_MOTION`, `doorAt`, `Duration`, `DURATION`, `earned`, `Ease`, `EASE`, `EASE_SPLINE`, `GLYPH_MOTION`, `Intent`, `Liveliness`, `LIVELINESS`, `LIVELINESS_KEY`, `livelinessStored`, `Motion`, `MOTION`, `motionFor`, `OPENING_MOTION`, `reading`, `REDUCED`, `rememberLiveliness`, `SAID`, `SETTLE`, `transition`, `TURN`, `TURN_AT`, `turns`, `useMotion`, `useStill`, `useStillness`
...
- `parts/forms.tsx` — `Agree`, `Choice`, `CodeEntry`, `DateInput`, `Dates`, `Dial`, `LongText`, `Lookup`, `MoneyInput`, `NamedAlready`, `NumberInput`, `NumberInputProps`, `OneOf`, `Option`, `PeriodId`, `PeriodInput`, `PERIODS`, `Picks`, `Said`, `SearchInput`, `SecretInput`, `Segmented`, `spanOf`, `Tags`, `Tail`, `TextInput`, `TextInputProps`, `TimeInput`
...
- `parts/state.tsx` — `Await`, `AwaitProps`, `both`, `ChartWaiting`, `FigureWaiting`, `FormWaiting`, `Loaded`, `Nothing`, `nothingIn`, `ready`, `RowsWaiting`, `TableWaiting`, `TextWaiting`, `TilesWaiting`, `trouble`, `Trouble`, `waiting`, `Working`
- `parts/surfaces.tsx` — `ActionRow`, `AmountRow`, `BrandTile`, `ControlRow`, `CopyRow`, `FieldRow`, `Group`, `GroupProps`, `Identity`, `Money`, `NavRow`, `NavRowProps`, `NoteRow`, `OfferRow`, `PersonRow`, `Place`, `QuickActions`, `SeeAll`, `StepRow`, `Swatch`, `TileGrid`, `ToggleRow`
