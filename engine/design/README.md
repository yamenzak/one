...
|---|---|---|
| `tokens/` | colour, type, spacing, motion, the chrome and hem rules | 58 |
...
**344 exports.** Every one is reachable as `import { … } from "@engine/design"`;
...
- `tokens/motion.ts` — `ARRIVE`, `ARRIVE_MARK`, `ARRIVE_MOTION`, `ARRIVE_RISE`, `arriveAt`, `BEAT`, `BLOCK_MOTION`, `DOOR_MOTION`, `doorAt`, `Duration`, `DURATION`, `Ease`, `EASE`, `GLYPH_MOTION`, `Intent`, `MOTION`, `REDUCED`, `transition`, `turns`, `useStill`
