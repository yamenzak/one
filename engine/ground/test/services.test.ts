...
beforeAll(async () => {
  ...
  await noteShardApp(directory(), "eu-2", "ground");
});
...
beforeEach(async () => {
  ...
  const made = await createTenant(directory(), {
    slug: "southwind", name: "Southwind", country: "DE", where: "eu", apps: ["ground"],
  });
...
const roster = async () => {
  await found(shard(), tenantId, "acc_owner" as never, "sam@example.com", { ground: "writer" });
  await invite(shard(), tenantId,
    { email: "alex@example.com", platformRole: "customer", appRoles: { ground: "reader" } },
    { platform: new Set(PLATFORM_ROLES.owner), inApp: async () => new Set(GROUND.access.roles.writer ?? []) },
    async () => GROUND.access.roles, { counts: ["owner", "manager", "staff"], allowed: -1 });
...
describe("being told something", () => {
  ...
  */
  it("reaches everybody who holds the key, and files a row for each", async () => {
    await roster();
    const told = await audienceFor(shard(), GROUND.notifications ?? {}, "ground", GROUND.access.roles,
      { ...dispatch, tenantId }, EVERY);
    expect(told.map((t) => t.email).sort()).toEqual(["alex@example.com", "sam@example.com"]);

    await fileNote(shard(), GROUND.notifications ?? {}, { ...dispatch, tenantId }, told);
...
describe("a note that leaves the process", () => {
  ...
  const raise = (type: string) => ({
    app: GROUND, tenantId, events: [type],
  ...
  */
  it("sends the workspace's own letter, and refuses one it could not fill", async () => {
    await roster();
    const book = GROUND.notifications ?? {};
    ...
    expect(await setLetter(shard(), book, tenantId, "note.published",
      { subject: "Hi {coach}", body: "Ground" })).toContain("unknown_variable");
...
describe("generating something and charging for it", () => {
  ...
  const ask = () => ({
    tenantId, appId: "ground", action: "note.draft", lane: "text" as const,
  ...
  */
  it("records where the credits went, and never what was written", async () => {
    ...
    expect(rows[0]).toMatchObject({ appId: "ground", action: "note.draft", ok: true, source: "usage" });
    ...
    expect(by).toEqual([{ key: "ground.note.draft", runs: 1, chargedMilli: rows[0]!.chargedMilli }]);
  });
