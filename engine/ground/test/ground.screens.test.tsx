 ...
 * is what makes "ground draws all of its screens" a fact rather than a claim.
 *
...
const html = (route: string) => renderToStaticMarkup(<GroundScreen route={route} />);

describe("ground draws every screen it declares", () => {
  it("names the same routes the manifest does", () => {
    expect([...GROUND_ROUTES].sort()).toEqual((GROUND.screens ?? []).map((s) => s.route).sort());
    expect(GROUND_ROUTES.length).toBeGreaterThan(3);
  });
  ...
  it("renders something for every declared route", () => {
    for (const route of GROUND_ROUTES) {
      ...
      const label = SUBJECT[route]
        ?? (GROUND.screens ?? []).find((s) => s.route === route)?.label
        ?? "";
