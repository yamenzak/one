...
beforeEach(async () => {
  ...
  await post("setup", "/api/me.tenant.create",
    { slug: "eastgate", name: "Eastgate", country: "DE", apps: ["ground"] }, owner);
});
  ...
  */
  it("lets a workspace give a feature to one of its own people", async () => {
    ...
    expect((await post("eastgate", "/api/member.invite",
      { email: "kim@example.com", platformRole: "customer", appRoles: { ground: "reader" } },
  ...
  */
  it("keeps one workspace's decision out of another", async () => {
    ...
    expect((await post("setup", "/api/me.tenant.create",
      { slug: "westgate", name: "Westgate", country: "DE", apps: ["ground"] }, owner)).status).toBe(200);
  ...
  it("carries every flag a product declares, and what each is for", async () => {
    ...
    const ground = seen.apps.find((a) => a.id === "ground")!;
    const declared = Object.values(GROUND.flags ?? {});
    expect(declared.length).toBeGreaterThan(0);
    for (const f of declared) {
      expect(ground.book[f.id]).toMatchObject({ label: f.label, why: f.why });
    }
  });
