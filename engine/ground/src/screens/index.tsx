/**
 * GROUND'S BROWSER HALF — eight screens, and the UI test ground.
 *
 ...
 *
 * ⚠️ AND THE SAMPLE WORLD IS THE APP'S OWN CONTENT, not a fixture. Ground is the
...
export { Note, Notes, People, Reports, Search, Settings, Shared, Start, Write };
 ...
 */
export const GROUND_ROUTES: readonly string[] =
 ...
 */
export function GroundScreen({ route, onGo }: {
  ...
  /* ⚠️ THE NAME COMES FROM THE MANIFEST, NOT FROM A STRING HERE. A screen titled
     ...
     second. */
  const title = (GROUND.screens ?? []).find((sc) => sc.route === route)?.label;
  ...
  switch (route) {
    ...
    /* ⚠️ DRAWN ON THE GROUND EVEN THOUGH THE NAV WOULD WITHHOLD IT, and the
       distinction is the point: `reachable` decides what is OFFERED, and the
       ground exists to photograph a screen. What keeps that honest is that
       nothing here enforces anything — the gate does, on every operation
       behind it. */
    case "/shared": return (
    ...
    default: return (
      <Notes
        title={title ?? "Notes"}
        of={notes}
        /* ⚠️ THE DECLARED SETTING, READ WHERE IT CHANGES SOMETHING. A setting
           saved by the platform and read by nothing is a control somebody
           presses and believes — `settings.test.mjs` is what refuses it — and
           the ground has no session, so the value it honours is the manifest's
           own fallback rather than a person's choice. */
        density={GROUND.settings?.["notes.density"]?.fallback as "comfortable" | "compact"}
        onNew={() => go("/write")}
        onOpen={() => go("/note")}
      />
    );
