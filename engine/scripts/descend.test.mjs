...
*/
const reachable = readdirSync(join(ENGINE, "one-hub/src/hub"))
  .filter((f) => f.endsWith(".tsx"))
  .some((f) => /at: "prefs"/.test(read(`one-hub/src/hub/${f}`)));

if (reachable) {
  ok("levels: your own preferences are offered somewhere you would look");
} else {
