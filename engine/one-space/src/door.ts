...
export interface Where {
   ...
   * The OneSpace asks constantly: opening a workspace is a route where it already
...
/**
 * ⚠️ THE SPACE TRAVELS BETWEEN DOORS AS AN ADDRESS, WHICH IS WHAT MAKES IT ONE
 ...
 * a full page load that arrives with OneSpace ALREADY OPEN on that workspace,
 * rather than a screen that loads and then refuses every call it makes.
 */
export const spaceAt = (origin: string, path: string): string => `${origin}${path}`;

/** Whether this door IS the given workspace — see `spaceAt` and `Where.slug`. */
export const isHere = (slug: string, where: Where): boolean =>
  where.kind === "tenant" && where.slug === slug;

/**
 * ⚠️ THE ONE PLACE THAT DECIDES WHAT THE SPACE IS FOR, GIVEN A DOOR. A pure
 * function, so it is a test rather than a click-through — and every kind is
 * answered, including the ones OneSpace is not, because "we serve this door" and
 * "we have a screen for it" drifting apart is how a page comes to render nothing
 * at all.
 */
export type Face = "signpost" | "space" | "create" | "centre" | "console" | "elsewhere";

export const faceFor = (kind: DoorKind): Face => {
  switch (kind) {
    case "signpost": return "signpost";
    case "account": return "space";
