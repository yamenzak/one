...
| [`@4dl/app-kit`](packages/app-kit/README.md) | the browser runtime: fetch + the three-way offline outcome, host resolution, prefixed storage, passkeys + the card, Stripe.js, Turnstile, and the inbox surface (bell + screen) | the queued-write pattern, the shell, the per-type icons |
