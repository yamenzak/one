/**
 * Scena's step-up confirmation codes — `@4dl/auth`'s mechanism, Scena's email.
 *
 ...
 * replayed to close a workspace. What is here is the message it arrives in.
 *
 * On the PLATFORM rail under Scena's identity, not the workspace's: "confirm
 * closing your workspace" is a message from us about the account, and wearing
 * the workspace's own branding on it would be a small lie in the one place a
 * reader is checking whether the mail is genuine.
 */

import { sendActionOtp as mint, verifyActionOtp as check } from "@4dl/auth";
import { emailShell, escapeHtml, sendEmail } from "./mailer.js";
  ...
  await mint(env, {
    ...
    deliver: async (code, actionLabel) => {
      const html = emailShell(
        ...
        { preheader: `${code} — your confirmation code` },
      );
      await sendEmail(env, {
        to: opts.email,
        subject: `${code} — confirm ${actionLabel}`,
        html,
        text: `Your confirmation code is ${code} (expires in 10 minutes).`,
      }).catch(() => undefined);
    },
  });
}
