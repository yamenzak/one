/**
 * THIS APP'S ACCENTS, held to the same bar the design system holds its own to.
 *
 * ⚠️ This file exists because of a bug that shipped. One app used `cardio`,
 * `nutrition`, `activity` and `sleep` — ANOTHER app's accent names — and defined
 * none of them. `@4dl/ui` resolves an unknown tone by convention, so nothing
 * errored: `var(--cardio)` was undefined and `bg-cardio-soft` was a class
 * Tailwind never emitted. Every icon badge in the app rendered uncoloured and
 * every test passed.
 *
