 ...
 *                          a centre thirty days past due got the ordinary
...
const tenantHost = (over: Record<string, unknown> = {}) => ({ role: "tenant", tenant: { name: "Klinik" }, ...over });

describe("the deployment-wide switch", () => {
  it("withholds the app at `full`, on every door", () => {
    for (const role of ["tenant", "setup", "root", "custom"]) {
      expect(pickScreen(false, CTX, { role, tenant: { name: "Klinik" }, maintenance: { level: "full" } })).toBe("maintenance");
    }
  });
...
describe("the standing ladder", () => {
  it("withholds the app for a BLOCKED centre", () => {
    expect(pickScreen(false, { active: { gate: { blocked: true } } }, tenantHost())).toBe("blocked");
  });

  it("still serves it on the READ-ONLY rung", () => {
    // Rung one withholds writes, not the product. A centre must be able to read
    // its own sterilisation records while an invoice is outstanding.
  ...
  it("does not reach the blocked screen for somebody with no membership here", () => {
    // `nostudio` comes first: they are not a member, so this centre's standing
    // is not their business and "pay your invoice" would be addressed to a
    // stranger.
    expect(pickScreen(false, { active: null }, tenantHost())).toBe("nostudio");
  });
});

describe("the order the two are checked in", () => {
  it("puts maintenance ABOVE a centre's own standing", () => {
