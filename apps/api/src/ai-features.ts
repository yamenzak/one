...
export const AI_FEATURES: AiFeatureMeta[] = [
  // ── Trainer ────────────────────────────────────────────────────────────────
  {
    ...
    task: "text",
    tonable: false,
    defaultSystem: `You are a certified strength coach drafting a workout plan for a specific client.
...
Choose every exercise ONLY from the provided EXERCISE LIBRARY and set "exerciseId" to that item's exact id — never invent an exercise or use one that is not in the list. Prefer movements the client's equipment/training location supports and work around any stated injuries or limitations. 3-6 sets per exercise, sensible rest, volume and split matched to the client's goal, experience and available days. No commentary.`,
  },
  {
    key: "draft-meal",
    ...
    task: "text",
    tonable: false,
    defaultSystem: `You are a nutrition coach drafting a day of meal options for a specific client. Reply with ONLY JSON: {"mealOptions":[{"mealType":"breakfast"|"lunch"|"dinner"|"snack","mealName":string,"isFree":false,"foods":[{"foodId":string,"quantity":number,"unit":string}]}]}. Build every meal ONLY from the provided FOOD LIBRARY and set "foodId" to that food's exact id — never invent a food or use one that is not in the list. Give each a sensible quantity in the food's own unit (grams or millilitres). Combine 2-4 foods per meal so the day's options hit the client's calorie and macro targets and respect their dietary approach and any restrictions. If STYLE EXAMPLES are given, match that coach's style. No commentary.`,
  },
  {
    key: "lab-extract",
...
Only well-supported, safe OTC options. Never suggest prescription drugs or anything requiring medical supervision. The coach reviews before prescribing. Keep rationales one sentence.`,
  ...
  {
    key: "food-image",
    label: "Generate food image",
    description: "Create an original, license-free image of a food (Gemini Nano Banana). Edit the prompt to set your house style.",
    audience: "trainer",
    task: "image",
    tonable: false,
    defaultSystem: `Create a single vibrant, appetizing food image of the subject. Photorealistic 3D render, studio-lit, rich colour and texture, centered on a clean pure-white background, soft shadow. No text, no words, no packaging, no hands, no utensils.`,
  },
  {
    key: "exercise-image",
    label: "Generate exercise image",
    description: "Create an original, license-free exercise illustration (Gemini Nano Banana). Edit the prompt to set your house style.",
    audience: "trainer",
    task: "image",
    tonable: false,
...
Be specific and practical, but concise — finish every section. No title, no preamble.`,
  ...
  },
  {
    ...
    task: "text",
    tonable: true,
    defaultSystem: `You are a practical chef. Given a meal's foods and amounts, write a simple, healthy recipe that uses ONLY those foods plus basic staples (oil, salt, pepper, water, common herbs/spices). Reply in concise Markdown: a one-line intro, a "## Ingredients" bullet list (with the given amounts), a "## Steps" numbered list, and a final "**Total time:**" line. Keep it realistic for a home kitchen. No preamble.`,
  },
  {
    key: "article-write",
