...
export const contentHubRoutes = new Hono<AppEnv>()
  ...
  .get("/resources", async (c) => {
    ...
    const rows = await c.env.DB.prepare(
      ...
      .bind(who.tenantId)
      .all<ResourceRow>();
    return c.json({ resources: (rows.results ?? []).map(view) });
  })
  ...
  .get("/resources/feed", async (c) => {
    ...
    return c.json({ resources: visible.map(view) });
  })
  ...
  .get("/resources/:id", async (c) => {
    ...
    return c.json({ resource: view(row) });
  })
  ...
  .post("/resources", async (c) => {
    ...
    await c.env.DB.prepare(
      ...
      .bind(id, who.tenantId, d.type, d.title, d.summary ?? null, d.bodyMd ?? null, d.coverUrl ?? null, d.topics.join(","), d.muscleGroups.join(","), d.durationMinutes ?? null, d.audience, j(d.assignedClientIds), who.userId, nowIso(), nowIso())
      .run();
...
export const marketplaceRoutes = new Hono<AppEnv>().get("/marketplace/:slug", async (c) => {
  const slug = c.req.param("slug");
  const org = await c.env.DB.prepare('SELECT id, name, slug, logo FROM "organization" WHERE slug = ?')
    .bind(slug)
    .first<{ id: string; name: string; slug: string; logo: string | null }>();
  ...
  const [settings, packages, posts] = await Promise.all([
    ...
    c.env.DB.prepare(
      ...
      .all(),
    c.env.DB.prepare(
      "SELECT id, type, title, summary, cover_url, published_at FROM resources WHERE tenant_id = ? AND status = 'published' AND audience = 'public' ORDER BY published_at DESC LIMIT 12",
    )
      .bind(org.id)
      .all(),
  ]);
