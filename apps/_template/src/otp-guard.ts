/**
 * THE ONE GATE IN FRONT OF THE SIGN-IN CODE — `@4dl/auth`'s policy, this app's
 * eligibility rule.
 *
 * ⚠️ **Mount this, or the door is bare.** `POST /api/auth/email-otp/send-verification-otp`
 * is the only way anybody signs in on this platform — there is no password
 * provider — and the shared guard was written, tested and shipped in `@4dl/auth`
 * while two of three apps simply did not mount it. Nothing failed anywhere. That
 * is the sharpest instance of the class of gap `docs/PLATFORM-AUDIT.md` is
 * about, and it is why this file exists in the template rather than being left
 * as an exercise.
 *
 * What the shared guard owns, in order: the human check → a 30-second
 * per-address cooldown → a per-source hourly ceiling → ELIGIBILITY (below) →
 * the deliverability pre-flight → forward to Better Auth's own handler.
 *
 ...
 *   the sign-in screen advances to "enter your code" forever, for a code that
 *   was never sent, and nobody — the operator included — can get in. That is a
 *   bootstrap trap no screen can escape, because reaching the screen that
 *   configures email needs a session, which needs a code.
 *
 * ── What is the APP's: who may receive a code at this door ──────────────────
 *
 * The rule below is the STRICT one — invitation-only, no self-registration
 * inside a tenant — because it is the safe default to loosen rather than the
 * unsafe one to remember to tighten. An app that sells self-signup replaces the
 * last branch with its own capacity check (Kova reserves a seat before saying
 * yes); an app that does not, keeps this.
 */
...
/**
 * Is there a live invitation to THIS tenant for this address?
 *
 ...
 * which is a tenant unable to add anybody. `status = 'pending'` and an unexpired
 * `expiresAt` are both required: a revoked or lapsed invitation must not keep
 * the door open, since revoking one is how a mistyped address is undone.
 */
...
export const otpSendGuard: MiddlewareHandler<AppEnv> = buildOtpGuard<Env, Auth, Branding>({
   ...
   * tenant's OWN domain is a different registrable domain covered by nothing. So
   * demanding a token there would not be a bot check — it would be a tenant that
  ...
  /**
   * The host decides which tenant this sign-in is for.
   *
   * The two hosts with no tenant behave differently, and both deliberately:
   *
   *  • `setup.` — no tenant yet, so there is nothing to check and anyone may
   *    request a code. That is how a new owner gets in, and closing it would
   *    make the product impossible to start.
   *  • the root — a signpost, not an app. There is no tenant to sign in TO, so a
  ...
  eligibility: async (c, email): Promise<EligibilityVerdict> => {
    ...
    /*
      ...
      an oracle for who belongs to a tenant. The `logEvent` is what an operator
      reads instead.
    */
