...
import {
  ArrowLeft, Save, Maximize2, Trash2, Loader2, Grid3x3, Magnet, Undo2, Redo2, Copy,
  ZoomIn, ZoomOut, MousePointer2, Layers as LayersIcon, ChevronUp, ChevronDown,
...
import {
  ...
  type Board, type Feed, type WeatherLocation, type BrandLogo,
} from "../api.js";
...
function hydrateNode(raw: unknown, i: number): WNode | null {
  ...
  return {
    ...
    x: nnum(o.x, 0), y: nnum(o.y, 0), w: nnum(o.w, 200), h: nnum(o.h, 120),
    rot: nnum(o.rot, 0), z: nnum(o.z, 10),
...
function Builder({ profileId }: { profileId: string }) {
  ...
  useEffect(() => {
    ...
    getBranding().then((v) => live && setLogos(v.logos ?? [])).catch(() => {});
    return () => { live = false; };
  ...
  const commit = useCallback((fn: (prev: WNode[]) => WNode[]) => { history.commit(fn); mark(); }, [history]);
  const live = useCallback((fn: (prev: WNode[]) => WNode[]) => { history.live(fn); mark(); }, [history]);
  ...
  const patchConfig = (id: string, p: Record<string, unknown>) => commit((prev) => prev.map((n) => (n.id === id ? { ...n, config: { ...n.config, ...p } } : n)));
  ...
  function duplicateSel() {
    ...
    const clones = picked.map((n) => ({ ...structuredClone(n), id: `${n.type}_${Date.now().toString(36)}${Math.random().toString(36).slice(2, 5)}`, x: n.x + 24, y: n.y + 24 }));
  ...
  const alignSel = (k: AlignKind) => applyMap(alignPatches(nodesRef.current.filter((n) => selRef.current.has(n.id)), k), false);
  const distributeSel = (axis: "h" | "v") => applyMap(distributePatches(nodesRef.current.filter((n) => selRef.current.has(n.id)), axis), false);

  /* --------------------------- clipboard -------------------------------- */
  const clip = useRef<WNode[]>([]);
  const copySel = () => { clip.current = nodesRef.current.filter((n) => selRef.current.has(n.id)).map((n) => structuredClone(n)); };
  const paste = () => {
    if (!clip.current.length) return;
    const clones = clip.current.map((n) => ({ ...structuredClone(n), id: `${n.type}_${Date.now().toString(36)}${Math.random().toString(36).slice(2, 5)}`, x: n.x + 24, y: n.y + 24 }));
    commit((prev) => [...prev, ...clones]);
    setSel(new Set(clones.map((c) => c.id)));
  };
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (meta && e.key.toLowerCase() === "z") { e.preventDefault(); e.shiftKey ? history.redo() : history.undo(); return; }
      if (meta && e.key.toLowerCase() === "y") { e.preventDefault(); history.redo(); return; }
      if (meta && e.key.toLowerCase() === "d") { e.preventDefault(); duplicateSel(); return; }
      if (meta && e.key.toLowerCase() === "c") { copySel(); return; }
      if (meta && e.key.toLowerCase() === "v") { paste(); return; }
      if (meta && e.key.toLowerCase() === "a") { e.preventDefault(); setSel(new Set(nodesRef.current.map((n) => n.id))); return; }
  ...
  const fit = () => { setZoom(1); const el = wrapRef.current; if (el) { el.scrollLeft = (el.scrollWidth - el.clientWidth) / 2; el.scrollTop = (el.scrollHeight - el.clientHeight) / 2; } };
  useEffect(() => {
    const el = wrapRef.current;
    if (!el) return;
    const onWheel = (e: WheelEvent) => { if (e.ctrlKey || e.metaKey) { e.preventDefault(); zoomBy(e.deltaY < 0 ? 1.1 : 1 / 1.1); } };
  ...
  useEffect(() => {
    ...
    const onStart = (e: TouchEvent) => { if (e.touches.length === 2) { startDist = dist(e.touches); startZoom = zoomRef.current; } };
  ...
  const selectOne = (id: string, additive: boolean) => {
    setSel((prev) => {
      if (additive) { const n = new Set(prev); n.has(id) ? n.delete(id) : n.add(id); return n; }
  ...
  useEffect(() => {
    ...
    const move = (e: MouseEvent) => {
      const st = marqueeStart.current, stage = stageRef.current;
      if (!st || !stage) return;
    ...
    const up = () => { marqueeStart.current = null; setMarquee(null); window.removeEventListener("mousemove", move); window.removeEventListener("mouseup", up); };
  ...
  const onBox = (patch: Partial<WNode>, phase: Phase) => {
    ...
    else if (phase === "move") { history.live((prev) => prev.map((n) => (n.id === single ? { ...n, ...patch } : n))); mark(); }
  };
  const onGroup = (patches: Record<string, Partial<WNode>>, phase: Phase) => {
    if (phase === "start") history.checkpoint();
    else if (phase === "move") { history.live((prev) => prev.map((n) => (patches[n.id] ? { ...n, ...patches[n.id] } : n))); mark(); }
  };
        ...
        {!layersOpen ? null : nodes.length === 0 ? (
          ...
          <div className="flex max-h-56 flex-col gap-0.5 overflow-y-auto">
            {[...nodes].sort((a, b) => b.z - a.z).map((n) => {
              ...
              return (
                <div key={n.id} onMouseDown={(e) => selectOne(n.id, e.shiftKey)}
                  className={cn("group flex cursor-pointer items-center gap-2 rounded-md px-2 py-1.5 text-sm", active ? "bg-primary/10 text-primary" : "hover:bg-accent")}>
                  ...
                  <button onClick={(e) => { e.stopPropagation(); reorder(n.id, "front"); }} aria-label="Bring to front" className="text-muted-foreground opacity-0 transition-opacity hover:text-foreground focus-visible:opacity-100 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100" title="Bring to front"><ChevronUp className="size-3.5" /></button>
                  <button onClick={(e) => { e.stopPropagation(); reorder(n.id, "back"); }} aria-label="Send to back" className="text-muted-foreground opacity-0 transition-opacity hover:text-foreground focus-visible:opacity-100 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100" title="Send to back"><ChevronDown className="size-3.5" /></button>
                  <button onClick={(e) => { e.stopPropagation(); setSel(new Set([n.id])); removeSel(); }} aria-label="Delete widget" className="text-muted-foreground opacity-0 transition-opacity hover:text-destructive focus-visible:opacity-100 group-hover:opacity-100 [@media(pointer:coarse)]:opacity-100" title="Delete"><Trash2 className="size-3.5" /></button>
                </div>
              );
            })}
          </div>
        ...
        {selIds.length > 1 ? (
          ...
          <>
            ...
            <div className="flex gap-1.5">
              <Button variant="outline" size="sm" className="flex-1" onClick={duplicateSel}><Copy className="size-3.5" /> Duplicate</Button>
              <Button variant="outline" size="sm" className="flex-1 text-destructive hover:text-destructive" onClick={removeSel}><Trash2 className="size-3.5" /> Delete</Button>
            </div>
          </>
        ) : (
        ...
        <div className="flex items-center gap-1.5 border-b px-2.5 py-2">
          ...
          <div className="mr-1 min-w-0">
            ...
            <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
              <span className="tabular-nums">{designW}×{designH}</span>
          ...
          <div className="ml-auto flex items-center gap-1">
            ...
            <Button onClick={save} disabled={saving || loading || loadError} size="sm" className="ml-1">
              {saving ? <Loader2 className="size-4 animate-spin" /> : <Save className="size-4" />}<span className="hidden sm:inline">{saving ? "Saving…" : "Save"}</span>
            </Button>
          </div>
        </div>
          ...
          <div ref={wrapRef} className="relative flex min-w-0 flex-1 items-center justify-center overflow-auto bg-[radial-gradient(circle_at_1px_1px,theme(colors.border)_1px,transparent_0)] bg-[size:22px_22px] p-6">
            {loading ? (
              ...
              <div ref={(el) => { stageRef.current = el; setStageEl(el); }} className="relative shrink-0 overflow-visible rounded-lg shadow-2xl ring-1 ring-black/40" style={{ width: W, height: H }}>
                ...
                >
                  {showGrid && <div className="pointer-events-none absolute inset-0" style={{ backgroundImage: "linear-gradient(oklch(1 0 0 / 0.06) 1px, transparent 1px), linear-gradient(90deg, oklch(1 0 0 / 0.06) 1px, transparent 1px)", backgroundSize: `${GRID * scale}px ${GRID * scale}px` }} />}
                  {[...nodes].sort((a, b) => a.z - b.z).map((n) => {
                    ...
                    return (
                      <div
                        key={n.id}
                        data-wid={n.id}
                        ref={(el) => { if (el) frameRefs.current.set(n.id, el); else frameRefs.current.delete(n.id); }}
                        onMouseDown={(e) => { e.stopPropagation(); selectOne(n.id, e.shiftKey); }}
                        style={{
                          position: "absolute", left: 0, top: 0, width: n.w * scale, height: n.h * scale,
                          transform: `translate(${n.x * scale}px, ${n.y * scale}px)${n.rot ? ` rotate(${n.rot}deg)` : ""}`,
                          transformOrigin: "center center",
                          zIndex: n.z, cursor: "move",
                          outline: active ? "none" : "1px dashed oklch(1 0 0 / 0.22)",
                          outlineOffset: 0,
                          backdropFilter: num(n.style.blur, 0) > 0 ? `blur(${num(n.style.blur, 0)}px)` : undefined,
                        }}
                      >
                        <div style={{ position: "absolute", top: 0, left: 0, width: n.w, height: n.h, transform: `scale(${scale})`, transformOrigin: "top left" }}>
                ...
                <div key={group} className="mb-4">
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:grid-cols-4">
                    {WIDGET_REGISTRY.filter((w) => w.group === group).map((w) => {
                      const usable = canUseWidget(w.type);
                      return (
                        <button key={w.type} onClick={() => { onPick(w.type); if (usable) setPaletteOpen(false); }}
                          className={cn("relative flex flex-col items-center gap-1.5 rounded-xl border p-3 text-center transition-colors hover:bg-accent active:bg-accent [&_svg]:size-6", !usable && "opacity-60")}>
...
function MultiPanel({ count, onAlign, onDistribute, onDuplicate, onDelete }: {
  count: number; onAlign: (k: AlignKind) => void; onDistribute: (a: "h" | "v") => void; onDuplicate: () => void; onDelete: () => void;
}) {
  const A = ({ k, icon, label }: { k: AlignKind; icon: React.ReactNode; label: string }) => (
    <Button variant="outline" size="icon" className="size-8" title={label} onClick={() => onAlign(k)}>{icon}</Button>
  );
  return (
    <div>
      ...
      <div className="mb-3 flex gap-1">
        <Button variant="outline" size="icon" className="size-8" title="Distribute horizontally" onClick={() => onDistribute("h")}><AlignHorizontalSpaceAround className="size-4" /></Button>
        <Button variant="outline" size="icon" className="size-8" title="Distribute vertically" onClick={() => onDistribute("v")}><AlignVerticalSpaceAround className="size-4" /></Button>
      </div>
      <Separator className="my-3" />
      <div className="flex gap-1.5">
        <Button variant="outline" size="sm" className="flex-1" onClick={onDuplicate}><Copy className="size-3.5" /> Duplicate</Button>
        <Button variant="outline" size="sm" className="flex-1 text-destructive hover:text-destructive" onClick={onDelete}><Trash2 className="size-3.5" /> Delete</Button>
      </div>
    </div>
  );
}
...
function SectionLabel({ icon, title }: { icon: React.ReactNode; title: string }) {
  return <div className="mb-3 flex items-center gap-1.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground">{icon}{title}</div>;
}
