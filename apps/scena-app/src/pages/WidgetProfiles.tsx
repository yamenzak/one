...
export function WidgetProfilesPage() {
  ...
  usePageChrome(
    {
      crumbs: [{ label: "Widget profiles" }],
      actions: canCreate ? [{ key: "new", label: "New profile", icon: <Plus className="size-4" />, onClick: () => setStarting(true) }] : [],
    },
    [canCreate],
  );
  ...
  return (
    <div>
      ...
      {error && !loading ? (
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
          {profiles.map((p) => (
            <Card key={p.id} onClick={() => navigate(`/widgets?profile=${p.id}`)} className="group cursor-pointer hover:border-primary/40 hover:shadow-md">
              ...
              {(canWrite || canDelete) && (
                <div className="-mr-1 flex shrink-0 gap-1 opacity-70 transition-opacity group-hover:opacity-100">
                  {canWrite && (
                    <Button
                      variant="ghost"
                      size="icon"
