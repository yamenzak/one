...
function toNode(raw: unknown, i: number): WNode | null {
  ...
  return {
    ...
    x: n(o.x, 0), y: n(o.y, 0), w: n(o.w, 200), h: n(o.h, 120),
    rot: n(o.rot, 0), z: n(o.z, 10 + i),
...
function ScenePreview({ nodes, designW = DESIGN_W, designH = DESIGN_H }: { nodes: WNode[]; designW?: number; designH?: number }) {
  ...
  return (
    <div ref={ref} className="relative w-full overflow-hidden bg-[linear-gradient(135deg,oklch(0.28_0.03_300),oklch(0.18_0.02_300))]" style={{ aspectRatio: `${designW} / ${designH}` }}>
      {nodes.length === 0 ? (
        ...
        <div className="absolute left-0 top-0 origin-top-left" style={{ width: designW, height: designH, transform: `scale(${scale})` }}>
          {[...nodes].sort((a, b) => a.z - b.z).map((n) => (
            <div key={n.id} className="absolute overflow-hidden" style={{ left: n.x, top: n.y, width: n.w, height: n.h, transform: n.rot ? `rotate(${n.rot}deg)` : undefined, transformOrigin: "center center", zIndex: n.z }}>
...
export function ProfileStartDialog({
  ...
  useEffect(() => {
    if (open) { setTab("template"); setPicked("breaking-red"); setName(""); setW(""); setH(""); setSrcId(profiles[0]?.id ?? ""); setAiPrompt(""); setAiErr(null); }
  }, [open, profiles]);

  // The layout designer runs on any text model (Gemini included).
  useEffect(() => {
    if (open && aiOn) listAiModels().then((ms) => setAiModels(ms.filter((m) => m.task === "text"))).catch(() => {});
  }, [open, aiOn]);

  async function open_(id: string) { onOpenChange(false); navigate(`/widgets?profile=${id}`); }
  ...
  async function createBlank() {
    setBusy(true);
    try {
      const designW = parseInt(w, 10), designH = parseInt(h, 10);
  ...
  async function createFromAi() {
    ...
    try {
      ...
      if (!r.ok || !Array.isArray(r.widgets) || !r.widgets.length) {
        throw new Error(r.detail || (r.error === "insufficient_credits" ? "Not enough AI credits for this generation." : r.error) || "The model returned no layout — try rephrasing.");
      }
      ...
      <DialogContent className="flex max-h-[88dvh] flex-col gap-0 overflow-hidden p-0 sm:max-w-5xl">
        ...
        <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col gap-0">
          <div className="px-5 pt-4">
            <TabsList>
              <TabsTrigger value="template"><LayoutTemplate className="size-4" /> Template</TabsTrigger>
              <TabsTrigger value="ai"><Sparkles className="size-4" /> Design with AI</TabsTrigger>
              <TabsTrigger value="blank"><SquareDashed className="size-4" /> Blank</TabsTrigger>
              <TabsTrigger value="duplicate" disabled={profiles.length === 0}><CopyPlus className="size-4" /> Duplicate</TabsTrigger>
            </TabsList>
          </div>
          ...
          <TabsContent value="ai" className="data-[state=inactive]:hidden">
            {!aiOn ? (
              <div className="px-5 py-10 text-center">
                <div className="mx-auto mb-3 grid size-11 place-items-center rounded-full bg-primary/10 text-primary"><Sparkles className="size-5" /></div>
                <div className="text-sm font-semibold">AI layout design isn’t in your plan</div>
                <p className="mx-auto mt-1 max-w-sm text-xs text-muted-foreground">Upgrade to describe a scene in words and have it composed out of real widgets. You can still start from a template or a blank canvas.</p>
              </div>
            ) : (
              <>
                <div className="space-y-3 px-5 py-5">
                  <div className="space-y-1">
                    ...
                    <Textarea autoFocus rows={4} placeholder="e.g. A lobby dashboard with a live clock, a welcome headline, and two KPI stats" value={aiPrompt} onChange={(e) => setAiPrompt(e.target.value)} className="resize-y" />
                  </div>
                  <div className="flex flex-wrap gap-1.5">
                    {AI_EXAMPLES.map((ex) => (
                      <button key={ex} type="button" onClick={() => setAiPrompt(ex)} className="rounded-full border px-2.5 py-1 text-left text-[11px] text-muted-foreground transition-colors hover:border-primary/40 hover:text-foreground">{ex}</button>
                    ))}
                  </div>
                  <div className="grid gap-3 sm:grid-cols-2">
                    <div className="space-y-1">
                      <label className="text-xs font-medium text-muted-foreground">Name <span className="font-normal">(optional)</span></label>
                ...
                <div className="flex items-center justify-end gap-2 border-t px-5 py-3">
                  ...
                  <Button onClick={createFromAi} disabled={busy || !aiPrompt.trim()}>
                    {busy ? <><Loader2 className="size-4 animate-spin" /> Designing…</> : <><Wand2 className="size-4" /> Generate &amp; open</>}
                  </Button>
                </div>
              </>
            )}
          ...
          <TabsContent value="blank" className="data-[state=inactive]:hidden">
            <div className="space-y-3 px-5 py-5">
              <div className="space-y-1">
                ...
                <Input autoFocus placeholder="e.g. Lobby overlay" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createBlank()} />
              </div>
              ...
              <p className="text-xs text-muted-foreground">Defaults to {DEFAULT_W}×{DEFAULT_H} if left blank.</p>
            </div>
            <div className="flex items-center justify-end gap-2 border-t px-5 py-3">
              <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button>
              <Button onClick={createBlank} disabled={busy}>{busy ? "Creating…" : "Create blank"}</Button>
            </div>
          </TabsContent>
          ...
          <TabsContent value="duplicate" className="data-[state=inactive]:hidden">
            <div className="space-y-3 px-5 py-5">
              ...
              <div className="space-y-1">
                ...
                <Input placeholder="Copy name" value={name} onChange={(e) => setName(e.target.value)} onKeyDown={(e) => e.key === "Enter" && createDuplicate()} />
              </div>
              <p className="text-xs text-muted-foreground">Copies the layout and canvas size; the original is untouched.</p>
            </div>
            <div className="flex items-center justify-end gap-2 border-t px-5 py-3">
              <Button variant="ghost" onClick={() => onOpenChange(false)}>Cancel</Button>
              <Button onClick={createDuplicate} disabled={busy || !srcId}>{busy ? "Duplicating…" : "Duplicate & open"}</Button>
            </div>
          </TabsContent>
        </Tabs>
      </DialogContent>
