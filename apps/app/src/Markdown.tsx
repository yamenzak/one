  ...
  const flushList = () => {
    ...
    const items = list.items.map((it, idx) => (
      <li key={idx} className="flex gap-2.5">
        {ordered
          ? <span className="mt-0.5 flex size-5 shrink-0 items-center justify-center rounded-full bg-primary/15 text-[0.7rem] font-semibold text-primary">{idx + 1}</span>
  ...
  for (let idx = 0; idx < lines.length; idx++) {
    ...
    if (heading) {
      ...
      if (level <= 1) {
        ...
        blocks.push(<h3 key={key++} className="mb-1.5 mt-5 text-[0.7rem] font-semibold uppercase tracking-[0.08em] text-muted-foreground">{content}</h3>);
      }
      continue;
    }
    if (uli || oli) {
