...
interface Intake { gender: "male" | "female" | null; dateOfBirth: string; heightCm: string; heightFt: string; heightIn: string; primaryGoal: string; workoutLocation: string; experienceLevel: string; activityLevel: string; dietaryApproach: string; availableDays: string[] }
const DAYS = ["Mon", "Tue", "Wed", "Thu", "Fri", "Sat", "Sun"];

export function Onboarding({ clientId, displayName, onDone }: { clientId: string; displayName: string; onDone: () => void }) {
  const [step, setStep] = useState(0);
  const [f, setF] = useState<Intake>({ gender: null, dateOfBirth: "", heightCm: "", heightFt: "", heightIn: "", primaryGoal: "maintain", workoutLocation: "gym", experienceLevel: "beginner", activityLevel: "moderate", dietaryApproach: "balanced", availableDays: ["Mon", "Wed", "Fri"] });
  ...
  const heightCm = units.height === "ft_in" ? (f.heightFt ? Math.round(feetInchesToCm(Number(f.heightFt), Number(f.heightIn || 0))) : null) : (f.heightCm ? Number(f.heightCm) : null);
