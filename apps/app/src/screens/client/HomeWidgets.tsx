/**
 * Client home widgets — the catalog a client can pick from for their Today
 * screen, including the live fasting tracker (ticking timer + zone), plus stat
 * tiles pulled from the Today bundle (streak, water, protein, calories, etc.).
 */

import { useCallback, useEffect, useState } from "react";
import { currentStreak, fmtVolume, fmtEnergy, kcalToDisplay, kgToDisplay, weightLabel, type UnitPrefs } from "@mossa/domain";
import { Badge, Chip, Button, IconBadge, Timer, Flame, Droplet, Beef, Dumbbell, FlaskConical, Weight, Pill } from "@mossa/ui";
...
export const DEFAULT_CLIENT_WIDGETS = ["fasting", "streak", "water", "weight"];
...
export const CLIENT_WIDGETS: WidgetDef<ClientWidgetData>[] = [
  ...
  { id: "streak", title: "Check-in streak", icon: Flame, render: (d) => <StatTile icon={Flame} tone="calories" value={d.bundle.checkInDates ? currentStreak(new Set(d.bundle.checkInDates), d.bundle.date) : 0} label="Day streak" /> },
  { id: "weight", title: "Weight trend (7d)", icon: Weight, render: (d) => <StatTile icon={Weight} tone="cardio" value={weightDelta(d.bundle.weightSeries, d.units)} label={`7-day ${weightLabel(d.units)}`} /> },
  { id: "water", title: "Water today", icon: Droplet, render: (d) => { const t = d.bundle.goal?.targets?.targetWaterMl ?? 2500; return <StatTile icon={Droplet} tone="hydration" value={fmtVolume(d.bundle.waterMl, d.units)} label="Water" sub={`of ${fmtVolume(t, d.units)}`} />; } },
  { id: "protein", title: "Protein today", icon: Beef, render: (d) => { const t = d.bundle.goal?.targets?.targetProteinG; return <StatTile icon={Beef} tone="protein" value={`${d.bundle.nutrition.proteinG} g`} label="Protein" sub={t ? `of ${t} g` : undefined} />; } },
  { id: "calories", title: "Calories left", icon: Flame, render: (d) => { const t = d.bundle.goal?.targets?.targetCalories ?? 0; const net = d.bundle.nutrition.calories - d.bundle.burnedKcal; return <StatTile icon={Flame} tone="calories" value={t > 0 ? kcalToDisplay(Math.max(0, t - net), d.units).toLocaleString() : fmtEnergy(net, d.units)} label={t > 0 ? "Calories left" : "Calories in"} />; } },
  { id: "burned", title: "Calories burned", icon: Flame, render: (d) => <StatTile icon={Flame} tone="cardio" value={kcalToDisplay(d.bundle.burnedKcal, d.units).toLocaleString()} label="Burned" /> },
  { id: "sets", title: "Sets logged today", icon: Dumbbell, render: (d) => <StatTile icon={Dumbbell} tone="activity" value={d.bundle.workout.loggedSets} label="Sets today" /> },
  { id: "labs", title: "Labs due", icon: FlaskConical, render: (d) => <StatTile icon={FlaskConical} tone="cardio" value={d.bundle.pendingLabs ?? 0} label="Labs due" /> },
  { id: "supplements", title: "Supplements today", icon: Pill, render: (d) => <SupplementsWidget clientId={d.clientId} /> },
];
...
function FastingWidget({ clientId }: { clientId: string }) {
  ...
  if (!fast) return <div className="col-span-2 h-28 animate-pulse rounded-2xl bg-surface-2" />;
  ...
  if (active) {
    ...
    return (
      <div className="relative col-span-2 overflow-hidden rounded-2xl bg-card p-4">
...
function SupplementsWidget({ clientId }: { clientId: string }) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const slots = s.supplements.reduce((n, x) => n + Math.max(1, x.schedule.length), 0);
      setTotal(slots); setTaken(l.taken.length);
    }).catch(() => setTotal(0));
  }, [clientId]);
  return <StatTile icon={Pill} tone="activity" value={total === null ? "—" : `${taken}/${total}`} label="Supplements" sub={total === 0 ? "none set" : "taken today"} />;
}
