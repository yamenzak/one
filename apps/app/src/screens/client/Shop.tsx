...
interface Sub { status: string; daysRemaining: number }
            ...
            <Stagger key={p.id}>
              <Card>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <div className="numeral shrink-0 text-xl font-bold">{p.one_time_price_cents ? `$${(p.one_time_price_cents / 100).toFixed(0)}` : <Badge tone="success">Free</Badge>}</div>
                </div>
                {p.one_time_price_cents ? <Button className="mt-4 w-full" onClick={() => void buy(p.id)}>Buy</Button> : <p className="mt-3 text-xs text-muted-foreground">Ask your coach to add this to your account.</p>}
              </Card>
            </Stagger>
          ))}
        </div>
